...
def command_tuple(text: str, synonym_map: dict, queue: deque):
    for line in text.splitlines():
        doc = nlp(line.lower())
        for token in doc:
            for form in (token.text, token.lemma_):
                if form in synonym_map:
                    action_name = synonym_map[form].__name__
                    complement = match_context(line, context, nlp, keyword_idx=token.i)
                    queue.append({"action": action_name, "complement": complement})
                    break
        #else:
            #action_search()
    return None

def exequeute(queue: deque, synonym_map: dict):   #haha execute + queue = exequeute...bref
    #les ordres de la queue en ordre    
    orders = {g.__name__: g for g in synonym_map.values()}
    while queue:
